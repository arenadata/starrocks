...
class PaimonChunkSink : public ConnectorChunkSink {
public:
    PaimonChunkSink(std::vector<std::string> partition_columns,
                    std::vector<std::unique_ptr<ColumnEvaluator>>&& partition_column_evaluators,
                    std::unique_ptr<PartitionChunkWriterFactory> partition_chunk_writer_factory, RuntimeState* state);

    ~PaimonChunkSink() override = default;

    void callback_on_commit(const CommitResult& result) override;
};

struct PaimonChunkSinkContext : public ConnectorChunkSinkContext {
    ~PaimonChunkSinkContext() override = default;
...
class PaimonChunkSinkProvider : public ConnectorChunkSinkProvider {
public:
    ~PaimonChunkSinkProvider() override = default;
