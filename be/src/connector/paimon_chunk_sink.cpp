...
PaimonChunkSink::PaimonChunkSink(std::vector<std::string> partition_columns,
...
void PaimonChunkSink::callback_on_commit(const CommitResult& result) {
    ...
    if (result.io_status.ok()) {
        ...
        TPaimonFileInfo paimon_file_info;
        paimon_file_info.__set_file_name(PathUtils::get_filename(result.location));
        paimon_file_info.__set_partition_path(PathUtils::get_parent_path(result.location));
        paimon_file_info.__set_record_count(result.file_statistics.record_count);
        paimon_file_info.__set_file_size_in_bytes(result.file_statistics.file_size);
        TSinkCommitInfo commit_info;
        commit_info.__set_paimon_file_info(paimon_file_info);
...
StatusOr<std::unique_ptr<ConnectorChunkSink>> PaimonChunkSinkProvider::create_chunk_sink(
        std::shared_ptr<ConnectorChunkSinkContext> context, int32_t driver_id) {
    auto ctx = std::dynamic_pointer_cast<PaimonChunkSinkContext>(context);
    ...
    auto partition_chunk_writer_ctx = std::make_shared<BufferPartitionChunkWriterContext>(
            BufferPartitionChunkWriterContext{
                    {file_writer_factory, location_provider, ctx->max_file_size, ctx->partition_column_names.empty()}});
    partition_chunk_writer_factory = std::make_unique<BufferPartitionChunkWriterFactory>(partition_chunk_writer_ctx);

    auto partition_column_evaluators = ColumnEvaluator::clone(ctx->partition_column_evaluators);
    return std::make_unique<connector::PaimonChunkSink>(ctx->partition_column_names,
