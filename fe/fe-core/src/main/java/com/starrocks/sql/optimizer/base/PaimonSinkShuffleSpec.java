...
public class PaimonSinkShuffleSpec implements ConnectorSinkShuffleSpec {
    private final PaimonTable paimonTable;

    public PaimonSinkShuffleSpec(PaimonTable paimonTable) {
    ...
    public PreOptimizeResult prepare(InsertStmt insertStmt,
        ...
        for (Column column : paimonTable.getPartitionColumns()) {
            int index = outputFullSchema.indexOf(column);
            Preconditions.checkState(index >= 0,
                    "Paimon partition column not found in output schema: " + column.getName());
